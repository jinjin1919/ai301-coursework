...
I work in Java/Spring Boot on the backend and TypeScript on the front end at production grade; those are the stacks I'm fastest in. I write Python comfortably and have prototyping experience on multi-agent systems with LangChain, LangGraph, CrewAI and BeeAI; I've called MCP servers but never written one. I'm aiming at AI engineering — agent orchestration and deploying agents to cloud infra — so rank Java or Python, agent-framework, and deployment-adjacent issues (packaging, Docker, CI config, API clients, retries/timeouts) above plain CRUD or frontend work. Avoid CSS and visual-design issues, languages I don't write (Go, Rust, C++), and anything needing paid API credits or a GPU to reproduce. When two issues are equally doable, prefer the one closer to agents; Java/TS familiarity is only a tiebreaker.
